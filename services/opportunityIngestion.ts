...
async function superteam(): Promise<NormalizedOpportunity[]> { const endpoints = [process.env.SUPERTEAM_EARN_LISTINGS_URL || 'https://earn.superteam.fun/api/listings']; const rows: Record<string, unknown>[] = []; for (const endpoint of endpoints) { const payload = await json(endpoint); const batch = Array.isArray(payload) ? payload : Array.isArray(payload?.listings) ? payload.listings : Array.isArray(payload?.data) ? payload.data : []; rows.push(...batch as Record<string, unknown>[]) } return rows.flatMap((row) => { const title = text(row.title); const sourceId = text(row.id) || text(row.slug); if (!title || !sourceId || !statusIsLive(row)) return []; const slug = text(row.slug); const sourceUrl = text(row.url) || (slug ? `https://earn.superteam.fun/listing/${slug}/` : 'https://earn.superteam.fun/'); const sponsor = row.sponsor as Record<string, unknown> | undefined; const deadline = date(row.deadline || row.endsAt || row.endDate); return [normalized({ source: 'superteam_earn', sourceId, sourceUrl, applicationUrl: text(row.applicationUrl) || sourceUrl, projectName: text(sponsor?.name) || text(row.project), title, description: text(row.description), opportunityType: text(row.type) || 'Bounty', category: text(row.type) || 'Bounty', rewardAmount: Number.isFinite(Number(row.rewardAmount ?? row.maxRewardAsk ?? row.minRewardAsk)) ? String(row.rewardAmount ?? row.maxRewardAsk ?? row.minRewardAsk) : null, rewardCurrency: text(row.token) || text(row.currency), rewardDescription: text(row.rewardText), deadline, ecosystem: text(row.ecosystem), location: text(row.location), remote: row.remote === true, skills: asStrings(row.skills), metadata: { providerStatus: String(row.status || 'OPEN') } })] }) }

async function railway(): Promise<NormalizedOpportunity[]> {
  const endpoint = process.env.RAILWAY_BOUNTIES_URL
  if (!endpoint) return []
  const payload = await json(endpoint)
  const rows = Array.isArray(payload) ? payload : Array.isArray(payload?.bounties) ? payload.bounties : Array.isArray(payload?.data) ? payload.data : []
  return (rows as Record<string, unknown>[]).flatMap((row) => {
    const title = text(row.title) || text(row.question)
    const sourceId = text(row.id) || text(row.slug) || text(row.url)
    const sourceUrl = text(row.url) || 'https://docs.railway.com/community/bounties'
    if (!title || !sourceId || !statusIsLive(row)) return []
    const deadline = date(row.deadline || row.endsAt || row.expiresAt)
    return [normalized({ source: 'railway', sourceId, sourceUrl, applicationUrl: sourceUrl, projectName: 'Railway', title, description: text(row.description) || text(row.body), opportunityType: 'Bounty', category: 'Bounty', rewardAmount: text(row.rewardAmount) || text(row.amount), rewardCurrency: text(row.rewardCurrency) || text(row.currency) || 'USD', rewardDescription: text(row.rewardDescription), deadline, ecosystem: 'Railway', location: text(row.location), remote: true, tags: ['railway', 'community'], metadata: { providerStatus: String(row.status || 'OPEN'), officialBoard: 'https://docs.railway.com/community/bounties' } })]
  })
}

async function github(): Promise<NormalizedOpportunity[]> { const query = encodeURIComponent('archived:false is:issue is:open (bounty OR reward OR paid OR grant)'); const rows: Record<string, unknown>[] = []; for (let page = 1; page <= 10; page += 1) { const payload = await json(`https://api.github.com/search/issues?q=${query}&per_page=100&page=${page}`); const batch = Array.isArray(payload?.items) ? payload.items : []; rows.push(...batch as Record<string, unknown>[]); if (batch.length < 100) break } return rows.flatMap((row) => { const title = text(row.title); const body = text(row.body) || ''; const url = text(row.html_url); const id = String(row.id || ''); const evidence = `${title} ${body}`.match(/\b(bounty|reward|paid|grant)\b/i); if (!title || !url || !id || !evidence) return []; const deadline = date(body.match(/(?:deadline|ends?)[^\n]*?(\d{4}-\d{2}-\d{2})/i)?.[1]); return [normalized({ source: 'github', sourceId: id, sourceUrl: url, applicationUrl: url, title, description: body, projectName: text((row.repository_url as string || '').split('/').pop()), opportunityType: /grant/i.test(`${title} ${body}`) ? 'Grant' : 'Bounty', category: /grant/i.test(`${title} ${body}`) ? 'Grant' : 'Bounty', rewardDescription: body.match(/[^\n]*(?:bounty|reward|paid)[^\n]*/i)?.[0], deadline, remote: true, tags: ['github'], metadata: { repositoryUrl: text(row.repository_url), qualifyingEvidence: evidence[0] } })] }) }
...
const providerAvailability: Record<string, string> = {
  railway: 'Official Railway bounty board is supported through RAILWAY_BOUNTIES_URL; no browser scraping or guessed endpoint is used.',
...
const providers = [
  ...
  { key: 'github', discover: github },
]

export async function syncOpportunitySources() { const results: Record<string, { ok: boolean; count: number; error?: string }> = {}; const attemptedAt = new Date(); for (const provider of providers) { try { const rows = await provider.discover(); for (const row of rows) await db.insert(opportunities).values({ ...(row as OpportunityInsert), lastSyncedAt: attemptedAt }).onConflictDoUpdate({ target: [opportunities.source, opportunities.sourceId], set: { ...row, lastSyncedAt: attemptedAt, updatedAt: attemptedAt } }); await db.insert(opportunitySources).values({ provider: provider.key, status: rows.length ? 'healthy' : 'unavailable', lastSuccessfulSync: rows.length ? attemptedAt : null, lastAttemptedSync: attemptedAt, error: providerAvailability[provider.key] || null, recordsDiscovered: rows.length, recordsValid: rows.length, updatedAt: attemptedAt }).onConflictDoUpdate({ target: opportunitySources.provider, set: { status: rows.length ? 'healthy' : 'unavailable', lastSuccessfulSync: rows.length ? attemptedAt : undefined, lastAttemptedSync: attemptedAt, error: providerAvailability[provider.key] || null, recordsDiscovered: rows.length, recordsValid: rows.length, updatedAt: attemptedAt } }); results[provider.key] = { ok: true, count: rows.length } } catch (error) { const message = error instanceof Error ? error.message : 'Source request failed'; await db.insert(opportunitySources).values({ provider: provider.key, status: 'error', lastAttemptedSync: attemptedAt, error: message, updatedAt: attemptedAt }).onConflictDoUpdate({ target: opportunitySources.provider, set: { status: 'error', lastAttemptedSync: attemptedAt, error: message, updatedAt: attemptedAt } }); results[provider.key] = { ok: false, count: 0, error: message } } } await db.update(opportunities).set({ liveStatus: 'expired', closedAt: attemptedAt, lastSyncedAt: attemptedAt, updatedAt: attemptedAt }).where(and(eq(opportunities.liveStatus, 'live'), lt(opportunities.deadline, attemptedAt))); return { results } }
