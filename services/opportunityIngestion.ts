...
function getSourcesClient() {
  ...
  if (!url || !key) {
    throw new Error('Supabase server configuration is missing. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SECRET_KEY.')
  }
...
export async function syncOpportunitySources() {
  ...
  const run = await getSourcesClient().from('opportunity_sync_runs').insert({ source: 'superteam_earn', status: 'RUNNING', started_at: startedAt }).select('id').maybeSingle()
  const runId = run.data?.id ?? null
  const results: Record<string, { ok: boolean; count: number; error?: string }> = {}
  try {
    const rows = await discoverSuperteam()
    const { error } = await getSourcesClient().from('opportunities').upsert(rows, { onConflict: 'source,source_id', ignoreDuplicates: false })
    if (error) throw error
    await getSourcesClient().from('opportunity_sources').upsert({ source: 'superteam_earn', name: 'Superteam Earn', enabled: true, api_status: 'healthy', last_success_at: startedAt, last_sync_at: startedAt, records_seen: rows.length, records_updated: rows.length, error_message: null }, { onConflict: 'source' })
    results.superteam_earn = { ok: true, count: rows.length }
    if (runId) await getSourcesClient().from('opportunity_sync_runs').update({ status: 'SUCCEEDED', finished_at: new Date().toISOString(), records_seen: rows.length, records_updated: rows.length }).eq('id', runId)
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown source error'
    await getSourcesClient().from('opportunity_sources').upsert({ source: 'superteam_earn', name: 'Superteam Earn', enabled: true, api_status: 'error', last_failure_at: startedAt, last_sync_at: startedAt, error_message: message }, { onConflict: 'source' })
    results.superteam_earn = { ok: false, count: 0, error: message }
    if (runId) await getSourcesClient().from('opportunity_sync_runs').update({ status: 'FAILED', finished_at: new Date().toISOString(), error_message: message }).eq('id', runId)
  }
