...
import { ArrowLeft, ChevronRight, Search, Wallet, X } from "lucide-react"
...
export default function FinancesScreen() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) return
      const { data: wallet } = await supabase.from("wallets").select("available").eq("user_id", data.user.id).eq("currency", "USD").maybeSingle()
      if (active) setBalance(Number(wallet?.available ?? 0))
    })
    return () => { active = false }
  }, [])
  const symbols = STOCK_UNIVERSE.map(({ symbol }) => `stock.${symbol}`)
  const { quotes, isLoading } = useLiveQuotes(symbols)
