...
const schema = z.object({
  ...
  currency: z.enum(["NGN", "USD", "USDT"]).default("NGN"),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) {
      const code = error.message.includes('ADMIN_ACCESS_REQUIRED') ? 'Admin access required.' : error.message.includes('Wallet not found') || error.message.includes('Wallet balance not found') ? 'The user does not have an active wallet for this currency.' : error.message
