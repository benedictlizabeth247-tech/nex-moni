...
export function CreditForm({ userId }: { userId: string }) {
  ...
  return <form onSubmit={submit} className="mt-5 grid gap-3 rounded-2xl border border-[#29413A] bg-[#101A18] p-4 sm:grid-cols-2"><p className="text-xs font-bold text-[#8DE0BD] sm:col-span-2">Production wallet funding/debit · every change is recorded in the audit ledger.</p>
    ...
    <label className="text-xs font-bold">Currency<select value={currency} onChange={e => setCurrency(e.target.value)} className="mt-2 w-full rounded-xl border border-[#29413A] bg-[#15231F] px-3 py-3 text-sm outline-none focus:border-[#55D6A7]"><option value="NGN">NGN</option><option value="USD">USD</option><option value="USDT">USDT</option></select></label>
