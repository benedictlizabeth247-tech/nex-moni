...
function OpportunityDetail({ item, onClose }: { item: DiscoveryOpportunity; onClose: () => void }) {
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-background"><div className="mx-auto min-h-screen max-w-screen-md bg-background"><header className="flex items-center justify-between border-b border-gray-100 bg-white px-6 py-5"><button onClick={onClose} aria-label="Close"><ArrowLeft size={20} /></button><span className="text-xs font-bold uppercase tracking-widest text-gray-400">Opportunity</span><button onClick={onClose} aria-label="Close"><X size={20} className="text-gray-400" /></button></header><div className="px-6 pb-12 pt-6"><div className="mb-6 flex h-40 items-center justify-center overflow-hidden rounded-2xl bg-gray-50">{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-full w-full object-cover" /> : <Globe2 size={38} className="text-gray-300" />}</div><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-accent">{item.source}</span><h1 className="mt-2 text-2xl font-bold leading-tight text-foreground">{item.title}</h1><p className="mt-2 text-sm text-gray-500">{item.organizationName}</p><div className="mt-5 flex flex-wrap gap-2">{[item.category, ...item.tags.slice(0, 3)].map((tag) => <Badge key={tag} variant="outline" className="rounded-full text-[10px]">{tag}</Badge>)}</div><div className="mt-8 space-y-6"><div><h2 className="text-sm font-bold text-foreground">About</h2><p className="mt-2 whitespace-pre-line text-sm leading-6 text-gray-600">{item.description || item.shortDescription}</p></div>{item.rewardLabel && <div><h2 className="text-sm font-bold text-foreground">Compensation</h2><p className="mt-2 text-sm text-gray-600">{item.rewardLabel}</p></div>}{item.deadline && <div><h2 className="text-sm font-bold text-foreground">Deadline</h2><p className="mt-2 text-sm text-gray-600">{item.deadline}</p></div>}</div><a href={item.sourceUrl} target="_blank" rel="noreferrer" className="mt-9 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-medium text-primary-foreground">View on {item.source}<ExternalLink size={16} /></a></div></div></div>
}
