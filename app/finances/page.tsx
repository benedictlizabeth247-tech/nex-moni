...
export default function FinancesScreen() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (active) {
        const authoritativeAvailable = Number(summary?.available_margin ?? wallet?.available ?? 0)
        setBalance(authoritativeAvailable)
