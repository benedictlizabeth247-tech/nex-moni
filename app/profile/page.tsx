...
export default function ProfileScreen() {
  ...
  return (
    <main className="min-h-screen bg-[#F2F1EF] pb-36 text-[#183A36] overflow-x-hidden">
      <header className="sticky top-0 z-40 border-b border-[#D9D6D2] bg-[#F2F1EF]/95 px-4 pt-7 pb-3 backdrop-blur">
        <div className="flex items-center justify-between">
          <button onClick={() => router.back()} className="h-10 w-10 rounded-xl bg-white border border-[#D9D6D2] flex items-center justify-center"><ArrowLeft size={18}/></button>
          <NexLogo />
          <div className="flex items-center gap-1">
            <button onClick={() => router.push('/notifications')} className="h-10 w-10 rounded-xl bg-white border border-[#D9D6D2] flex items-center justify-center"><Bell size={18}/></button>
            <button onClick={() => setShowSearch(v=>!v)} className="h-10 w-10 rounded-xl bg-white border border-[#D9D6D2] flex items-center justify-center"><Search size={18}/></button>
          </div>
        </div>
      ...
      <div className="space-y-4 px-4 py-4">
        <Card className="rounded-[28px] border-[#D9D6D2] bg-white p-5 shadow-none">
          <div className="flex items-center gap-3">
            <div className="h-12 w-12 rounded-2xl bg-[#F1E4E0] flex items-center justify-center text-[#8D554D] overflow-hidden">
        ...
        <Card className="rounded-[26px] border-[#D9D6D2] bg-white p-2 shadow-none">
          <div className="flex gap-1 overflow-x-auto">
            {(['overview','spot','futures','funding'] as AssetTab[]).map(t=><button key={t} onClick={()=>setTab(t)} className={cn('min-w-[82px] flex-1 rounded-xl py-2.5 text-[10px] font-black capitalize',tab===t?'bg-[#F1E4E0] text-[#8D554D]':'text-[#737A7D]')}>{t}</button>)}
          </div>
        </Card>

        <section className="grid grid-cols-3 gap-2">
          {[['Funding',balances.funding,'funding'],['Spot',balances.spot,'spot'],['Futures',balances.futures,'futures']].map(([label,value,key])=><button key={String(key)} onClick={()=>setTab(key as AssetTab)} className={cn('rounded-2xl border p-3 text-left bg-white',tab===key?'border-[#8D554D]':'border-[#D9D6D2]')}><p className="text-[9px] text-[#737A7D]">{label}</p><p className="mt-1 text-[13px] font-black truncate">{showBalance?money(Number(value)):'••••'}</p></button>)}
        </section>

        <Card className="rounded-[26px] border-[#D9D6D2] bg-white p-4 shadow-none">
          <div className="flex items-center justify-between"><div><p className="text-[9px] uppercase tracking-[.16em] text-[#737A7D]">{tab === 'overview' ? 'Overview' : `${tab} balance`}</p><p className="mt-1 text-[24px] font-black">{showBalance?money(displayed):'••••••'}</p></div><button onClick={()=>router.push('/assets')} className="rounded-xl bg-[#F1E4E0] px-3 py-2 text-[9px] font-black text-[#8D554D]">Full assets</button></div>
          <div className="mt-4 grid grid-cols-2 gap-2">
        ...
        <Card className="rounded-[26px] border-[#D9D6D2] bg-white p-4 shadow-none">
          <div className="flex items-center justify-between"><div><p className="text-[11px] font-black">Recent account activity</p><p className="text-[9px] text-[#737A7D]">Deposits, transfers, payments and wallet events</p></div><button onClick={()=>router.push('/transactions')}><ChevronRight size={17} className="text-[#737A7D]"/></button></div>
          <div className="mt-3 space-y-2">{transactions.slice(0,5).map(tx=><div key={tx.id} className="flex items-center justify-between rounded-xl bg-[#F5F8F7] p-3"><div><p className="text-[10px] font-bold">{tx.title}</p><p className="text-[8px] text-[#737A7D]">{new Date(tx.created_at).toLocaleString()}</p></div><span className={cn('text-[10px] font-black',tx.amount>=0?'text-[#16A36A]':'text-[#B94A48]')}>{tx.amount>=0?'+':''}{money(tx.amount)}</span></div>)}{!transactions.length&&<p className="rounded-xl bg-[#F5F8F7] p-4 text-[9px] text-[#737A7D]">No account activity yet.</p>}</div>
        </Card>

        <section className="space-y-2">
          <label className="ml-1 text-[9px] font-black uppercase tracking-[.16em] text-[#737A7D]">Account</label>
        ...
        <section id="profile-preferences" className="space-y-2 scroll-mt-24">
          ...
          <Card className="overflow-hidden rounded-[26px] border-[#D9D6D2] bg-white shadow-none divide-y divide-[#F2F1EF]">
            ...
            <div className="flex items-center justify-between p-4"><div><p className="text-[11px] font-bold">Biometric login</p><p className="text-[9px] text-[#737A7D]">Use device security for sign-in</p></div><Switch checked={biometrics} onCheckedChange={setBiometrics}/></div>
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3"><div className="h-9 w-9 rounded-xl bg-[#F1E4E0] text-[#8D554D] flex items-center justify-center">{theme === 'dark' ? <Moon size={16}/> : <Sun size={16}/>}</div><div><p className="text-[11px] font-black">Appearance</p><p className="text-[9px] text-[#737A7D]">Choose light or dark mode across the app</p></div></div>
              <div className="flex rounded-xl bg-[#F5F8F7] p-1">
                <button onClick={() => changeTheme('light')} className={cn('flex h-8 items-center gap-1 rounded-lg px-2.5 text-[9px] font-black', theme === 'light' ? 'bg-white text-[#8D554D] shadow-sm' : 'text-[#737A7D]')}><Sun size={13}/> Light</button>
                <button onClick={() => changeTheme('dark')} className={cn('flex h-8 items-center gap-1 rounded-lg px-2.5 text-[9px] font-black', theme === 'dark' ? 'bg-[#16352F] text-white shadow-sm' : 'text-[#737A7D]')}><Moon size={13}/> Dark</button>
              </div>
            </div>
          </Card>
        </section>
...
function ProfileOption({icon,title,subtitle,onClick}:{icon:React.ReactNode,title:string,subtitle:string,onClick?:()=>void}){
 return <button onClick={onClick} className="w-full flex items-center justify-between p-4 text-left hover:bg-[#F8FAF9]"><div className="flex items-center gap-3"><div className="h-9 w-9 rounded-xl bg-[#F1E4E0] text-[#8D554D] flex items-center justify-center">{React.cloneElement(icon as React.ReactElement<{ size?: number }>,{size:16})}</div><div><p className="text-[11px] font-black">{title}</p><p className="mt-0.5 text-[9px] text-[#737A7D]">{subtitle}</p></div></div><ChevronRight size={16} className="text-[#A4AFAB]"/></button>
}
