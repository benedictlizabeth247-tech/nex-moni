  ...
  return (
    <main className="min-h-screen bg-background pb-28">
      <header className="sticky top-0 z-30 border-b border-gray-100 bg-white/95 px-6 pb-5 pt-6 backdrop-blur">
        ...
        <div className="mt-4 flex max-w-sm items-start gap-2 rounded-xl border border-primary/15 bg-primary/5 px-3 py-2.5 text-xs leading-5 text-foreground/75">
          <Globe2 size={15} className="mt-0.5 shrink-0 text-primary" />
          <span><strong className="font-semibold text-foreground">Superteam feed</strong> is synced from the official API. Listings, rewards, deadlines, links, and project profile images come directly from each live job post.</span>
        </div>
      </header>
...
function displaySource(source: string) {
  return source === "superteam_earn" ? "Superteam" : source.replaceAll("_", " ")
}

function OpportunityCard({ item, onClick }: { item: DiscoveryOpportunity; onClick: () => void }) {
  const profileImage = item.projectProfileUrl ? item.imageUrl : item.creatorAvatarUrl || item.imageUrl
  return <button onClick={onClick} className="flex w-full items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 text-left shadow-nex-soft transition-transform active:scale-[.99]">
    <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-primary/10 bg-primary/5">{profileImage ? <img src={profileImage} alt={`${item.organizationName} profile`} className="h-full w-full object-cover" /> : <Globe2 size={22} className="text-gray-300" />}</div>
    <div className="min-w-0 flex-1"><div className="mb-1 flex items-center gap-2"><span className="truncate text-[10px] font-bold uppercase tracking-wider text-accent">{displaySource(item.source)}</span>{item.source === "superteam_earn" && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-semibold text-primary">Official API</span>}</div><h3 className="truncate text-sm font-bold text-foreground">{item.title}</h3><p className="mt-1 truncate text-xs text-gray-500">{item.organizationName} · {item.category}</p>{item.sourceProfileUrl && <a href={item.sourceProfileUrl} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()} className="mt-1 inline-block text-[10px] font-semibold text-accent">View project profile</a>}<div className="mt-2 flex items-center gap-3 text-[10px] text-gray-400">{item.rewardLabel && <span className="font-semibold text-primary">{item.rewardLabel}</span>}{item.deadline && <span className="flex items-center gap-1"><Clock3 size={11} /> {item.deadline}</span>}</div></div><ChevronRight size={18} className="shrink-0 text-gray-300" />
  </button>
}

function OpportunityDetail({ item, onClose }: { item: DiscoveryOpportunity; onClose: () => void }) {
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-background"><div className="mx-auto min-h-screen max-w-screen-md bg-background"><header className="flex items-center justify-between border-b border-gray-100 bg-white px-6 py-5"><button onClick={onClose} aria-label="Close"><ArrowLeft size={20} /></button><span className="text-xs font-bold uppercase tracking-widest text-gray-400">Opportunity</span><button onClick={onClose} aria-label="Close"><X size={20} className="text-gray-400" /></button></header><div className="px-6 pb-12 pt-6"><div className="mb-6 flex h-40 items-center justify-center overflow-hidden rounded-2xl bg-gray-50">{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-full w-full object-cover" /> : <Globe2 size={38} className="text-gray-300" />}</div><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-accent">{displaySource(item.source)}{item.source === "superteam_earn" ? " · Official API" : ""}</span><h1 className="mt-2 text-2xl font-bold leading-tight text-foreground">{item.title}</h1><p className="mt-2 text-sm text-gray-500">{item.organizationName}</p><div className="mt-5 flex flex-wrap gap-2">{[item.category, ...item.tags.slice(0, 3)].map((tag) => <Badge key={tag} variant="outline" className="rounded-full text-[10px]">{tag}</Badge>)}</div><div className="mt-8 space-y-6"><div><h2 className="text-sm font-bold text-foreground">About</h2><p className="mt-2 whitespace-pre-line text-sm leading-6 text-gray-600">{item.description || item.shortDescription}</p></div>{item.rewardLabel && <div><h2 className="text-sm font-bold text-foreground">Compensation</h2><p className="mt-2 text-sm text-gray-600">{item.rewardLabel}</p></div>}{item.deadline && <div><h2 className="text-sm font-bold text-foreground">Deadline</h2><p className="mt-2 text-sm text-gray-600">{item.deadline}</p></div>}</div><a className="mt-9 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground" href={item.applicationUrl || item.sourceUrl} target="_blank" rel="noreferrer">{item.applicationUrl ? "Apply" : `View on ${item.source}`}<ExternalLink size={16} /></a></div></div></div>
}
