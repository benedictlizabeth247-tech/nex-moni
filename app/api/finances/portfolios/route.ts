...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select('id,user_id,name,description,strategy,risk_level,is_public,performance_30d,created_at,updated_at,portfolio_holdings(id,ticker,quantity,entry_price,current_price,currency,updated_at)')
    .or(`is_public.eq.true,user_id.eq.${auth.user.id}`)
