...
export default function AdminSection({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <main className="min-h-screen bg-[#101A18] p-4 text-[#EAF4F0] md:p-8"><div className="mx-auto max-w-6xl">
    ...
    <p className="mt-8 text-xs font-bold uppercase tracking-[0.2em] text-[#55D6A7]">Live Neon operations</p><h1 className="mt-2 text-3xl font-black">{title}</h1><p className="mt-2 text-sm leading-6 text-[#9BB8AF]">Automatically refreshed from the operational database every 10 seconds.</p>
    {section === "users" && <p className="mt-4 flex items-center gap-2 rounded-xl bg-[#153B2F] p-3 text-xs text-[#8DE0BD]"><Users size={15}/> {payload.users?.length || 0} registered Neon users</p>}
