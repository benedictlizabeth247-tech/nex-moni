...
export default function FundAccountPage() {
  ...
  const [method, setMethod] = useState<Method>('fiat'); const [session, setSession] = useState<DepositSession|null>(null)
  const [banks, setBanks] = useState<any[]>([]); const [loading, setLoading] = useState(true); const [submitting, setSubmitting] = useState(false)
  const [stage, setStage] = useState<'choose'|'details'|'assigning'|'pending'>('choose')
  ...
  const selectedReceiving = session?.receivingAccounts?.[0]
  ...
  const submitFiat = async () => {
    ...
    try {
      ...
      setSession({ id: result.depositId, bankName: result.merchant.bankName, accountNumber: result.merchant.accountNumber, accountName: result.merchant.accountName, receivingAccounts: [{ bankName: result.merchant.bankName, accountNumber: result.merchant.accountNumber, accountName: result.merchant.accountName }], expiryTime: new Date(result.merchant.expiresAt), status: 'active' })
      setStage('pending')
    ...
    <div className="mx-auto max-w-[520px] space-y-4 px-4 py-5">
      {stage === 'choose' && <>
        ...
        {method === 'fiat' && <Card className="rounded-[28px] border-[#E1D5D1] bg-[#FFFDFB] p-5 shadow-none"><div className="mb-4 flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-[#8E7772]"><span className="rounded-full bg-[#D86F68] px-2 py-1 text-white">1</span> Bank transfer deposit</div>
          ...
          <div className="mt-4 rounded-2xl border border-[#E1D5D1] bg-[#FAF7F5] p-4"><p className="text-[10px] font-black uppercase tracking-widest text-[#342A28]">Merchant account assigned after Continue</p><p className="mt-2 text-[11px] leading-5 text-[#6F5C57]">NexMonie will check active settlement accounts, capacity, limits, and availability before showing the bank details for this transaction.</p></div>
      ...
      {stage === 'assigning' && <Card className="rounded-[30px] border-[#E1D5D1] bg-[#FFFDFB] p-7 shadow-none"><div className="h-1 overflow-hidden rounded-full bg-[#F1E2DE]"><div className="h-full w-2/3 animate-pulse rounded-full bg-[#D86F68]"/></div><p className="mt-7 text-[11px] font-black uppercase tracking-[.16em] text-[#8E7772]">Finding an available merchant</p><h2 className="mt-2 text-[23px] font-black">Checking settlement accounts</h2><div className="mt-6 space-y-3 text-left text-[13px] text-[#342A28]"><p>✓ Deposit amount received</p><p>✓ Checking active merchant accounts</p><p>✓ Checking transaction capacity</p><p className="font-black text-[#D86F68]">● Matching your deposit amount</p><p className="text-[#A99A95]">○ Preparing secure payment instructions</p></div></Card>}
      {stage === 'pending' && <Card className="rounded-[30px] border-[#E1D5D1] bg-[#FFFDFB] p-7 shadow-none"><CheckCircle2 size={52} className="text-[#B86B64]"/><p className="mt-4 text-[11px] font-black uppercase tracking-[.16em] text-[#8E7772]">Merchant account assigned</p><h2 className="mt-1 text-[25px] font-black">{selectedReceiving?.bankName}</h2><p className="mt-1 text-[13px] text-[#6F5C57]">Active merchant</p><div className="mt-5 rounded-2xl bg-[#F8F2F0] p-4 text-left"><p className="text-[11px] text-[#8E7772]">Account name</p><b className="text-[16px]">{selectedReceiving?.accountName}</b><p className="mt-3 text-[11px] text-[#8E7772]">Account number</p><div className="flex items-center justify-between gap-3"><b className="text-[22px] tracking-wide">{selectedReceiving?.accountNumber}</b><button onClick={() => selectedReceiving && copy(selectedReceiving.accountNumber, true)} className="flex h-10 items-center gap-2 rounded-xl bg-[#342A28] px-3 text-[11px] font-black text-white"><Copy size={14}/>{copiedAccount ? 'Copied' : 'Copy'}</button></div><p className="mt-3 text-[11px] text-[#8E7772]">Bank branch</p><b className="text-[16px]">{(session as any)?.branchName ?? '—'}</b></div><div className="mt-4 rounded-2xl bg-[#F9E8E5] p-4"><p className="text-[11px] font-black uppercase tracking-widest text-[#8E7772]">Complete transfer within</p><p className="mt-1 text-[34px] font-black tabular-nums">{countdownLabel}</p><p className="text-[12px] text-[#6F5C57]">Transfer exactly ₦{fiatAmount.toLocaleString('en-NG', { minimumFractionDigits: 2 })} to the account shown above.</p></div><p className="mt-4 text-[12px] leading-5 text-[#6F5C57]">Only transfer to the merchant account currently displayed for this active transaction. Do not use a previously displayed account after this window expires.</p><button onClick={() => router.push('/transactions')} className="mt-5 w-full rounded-2xl bg-[#342A28] py-3 text-[12px] font-black text-white">I&apos;ve Made the Transfer</button></Card>}
    </div>
    <BottomNav/>
  </main>
}
