...
type Position = { id: string; symbol: string; side: string; quantity: string | number; entryPrice: string | number; markPrice: string | number; unrealizedPnl: string | number; status: string }
type Overview = { totalBalance: number; totalLocked: number; totalAvailable: number; unrealizedPnl: number; funding: Account; spot: Account; futures: Account; positions: { spot: Position[]; futures: Position[] } }
...
export default function AssetsPage() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-background pb-32 text-foreground">
    ...
    <div className="mx-auto max-w-2xl space-y-4 p-4">
      ...
      <AccountCard title="Spot account" icon={<TrendingUp size={20} />} account={account("spot")} description="Available balance can be used for spot trading or transfers." actions={<button onClick={() => router.push('/spot')} className="rounded-xl bg-primary px-3 py-2 text-xs font-black text-primary-foreground">Trade spot</button>} positions={data?.positions.spot} />
      <AccountCard title="Futures account" icon={<LockKeyhole size={20} />} account={account("futures")} description="Open position margin is locked; the remainder is available." actions={<button onClick={() => router.push('/futures')} className="rounded-xl bg-primary px-3 py-2 text-xs font-black text-primary-foreground">Trade futures</button>} positions={data?.positions.futures} />
...
function AccountCard({ title, icon, account, description, actions, positions = [] }: { title: string; icon: ReactNode; account: Account; description: string; actions?: ReactNode; positions?: Position[] }) {
  return <section className="rounded-3xl border border-border bg-card p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-2xl bg-primary/10 text-primary">{icon}</div><div><h2 className="font-black">{title}</h2><p className="text-xs text-muted-foreground">{description}</p></div></div>{actions && <div className="flex gap-2">{actions}</div>}</div><div className="mt-5 grid grid-cols-3 gap-2"><Metric label="Balance" value={account.balance} /><Metric label="Locked" value={account.locked ?? 0} /><Metric label="Available" value={account.available} accent /></div>{positions.length > 0 && <div className="mt-4 space-y-2 border-t border-border pt-3"><div className="flex items-center justify-between"><p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Open positions</p><span className="text-[10px] font-bold text-primary">Live equity included</span></div>{positions.map((position) => <div key={position.id} className="flex items-center justify-between rounded-2xl bg-muted/60 px-3 py-2 text-xs"><span className="font-black">{position.symbol} <span className="text-muted-foreground">{position.side.toUpperCase()} · {position.quantity}</span></span><span className={Number(position.unrealizedPnl) >= 0 ? "font-black text-primary" : "font-black text-destructive"}>{Number(position.unrealizedPnl) >= 0 ? "+" : ""}{money(Number(position.unrealizedPnl))}</span></div>)}</div>}</section>
}
